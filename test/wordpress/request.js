(function(){
    ...
    var appsecret = "EBAnx3PFZbwi3sC7";
    var queryStr;
    var token;
    ...
    $('.oauth').on('click', function(){
        ...
        if(Tools.getParams('oauth_token')){
            param.oauth_token = Tools.getParams('oauth_token');
            param.oauth_verifier = Tools.getParams('oauth_verifier');
        }
        queryStr = request(true);
        accessToken(queryStr);
    })

    $('.oauth2').on('click', function(){
        parameters = {
            oauth_consumer_key: appkey,
            oauth_nonce: new Date().getTime(),
            oauth_signature_method: "HMAC-SHA1",
        ...
        queryStr = request(true, 2);
        accessToken(queryStr);
    })

    function request(access, type){

        var req_url = 'http://openapi.qzone.qq.com/oauth/qzoneoauth_request_token';
        if(access){
